...
bool AccessFrequencyComparator(std::vector<uint16_t> i,
...
TypeId SafApplication::GetTypeId(void) {
  static TypeId tid =
      TypeId("ns3::SafApplication")
          ...
          .AddAttribute("Port", "The application port", UintegerValue(5000),
          ...
          .AddAttribute("DataSize", "The number of bytes in each data object.",
                        UintegerValue(30),
          ...
          .AddAttribute("StorageSpace",
                        ...
                        MakeUintegerChecker<uint16_t>())
          .AddAttribute(
              "accessFrequencyMode", "The access frequency type, 1, 2, or 3",
              UintegerValue(10),
...
void SafApplication::StartApplication(void) {
  ...
  if (m_socket_send == 0) {
    ...
    m_socket_send->Connect(
        InetSocketAddress(Ipv4Address::GetBroadcast(), m_port));
  }
  m_socket_recv->SetRecvCallback(
      MakeCallback(&SafApplication::HandleRequest, this));
  m_socket_send->SetRecvCallback(
      MakeCallback(&SafApplication::HandleResponse, this));
  m_socket_send->SetAllowBroadcast(true);
  m_socket_send->SetIpTtl(
      2); // or should this be 0? to only send to 1 hop peers
  ...
  for (uint16_t i = 1; i <= m_total_data_items; i++) {
    ...
    double lookupDelay = m_reallocation_period.GetSeconds() -
                         (m_reallocation_period.GetSeconds() * accessFrequency);
  ...
  sort(m_access_frequencies.begin(), m_access_frequencies.end(),
       AccessFrequencyComparator);

  // schedule first reallocation event
  m_reallocation_event = Simulator::Schedule(
      m_reallocation_period, &SafApplication::RunReplication, this);
...
void SafApplication::StopApplication() {
  ...
  for (std::set<uint32_t>::iterator it = m_pending_lookups.begin();
       it != m_pending_lookups.end(); it++) {
    NS_LOG_INFO("TODO: sim ended before application request ID "
                << *it << " timed out");
  }

  // check to see if it is still in the pending lookup list
  for (std::set<uint32_t>::iterator it = m_pending_reallocations.begin();
       it != m_pending_reallocations.end(); it++) {
    NS_LOG_INFO("TODO: sim ended before reallocation request ID "
                << *it << " timed out");
  }
...
void SafApplication::ScheduleFirstLookups() {
  ...
  for (uint16_t i = 1; i <= m_total_data_items; i++) {
    ...
    Simulator::Schedule(Seconds(dt), &SafApplication::ScheduleNextLookup, this,
                        i);
  }
}
...
void SafApplication::HandleRequest(Ptr<Socket> socket) {
  ...
  while ((packet = socket->RecvFrom(from))) {
    ...
    if (payload[0] == uint8_t(MessageType::lookup)) {
      ...
      // collisions
      if (isReplication) {
        if (!m_realloc_rcv_CB.IsNull())
          m_realloc_rcv_CB(dataID, GetNode()->GetId());
      } else {
        if (!m_lookup_rcv_CB.IsNull())
          m_lookup_rcv_CB(dataID, GetNode()->GetId());
      }
      ...
      ResponseMessage r =
          ResponseMessage(requestID, sentAt, isReplication, item);
      Ptr<Packet> responsePacket = r.ToPacket();

      if (isReplication) {
        if (!m_realloc_rsp_sent_CB.IsNull())
          m_realloc_rsp_sent_CB(dataID, GetNode()->GetId());
      } else {
        if (!m_lookup_rsp_sent_CB.IsNull())
          m_lookup_rsp_sent_CB(dataID, GetNode()->GetId());
      }
...
void SafApplication::HandleResponse(Ptr<Socket> socket) {
  ...
  while ((packet = socket->RecvFrom(from))) {
    ...
    NS_LOG_INFO("At time " << Simulator::Now().GetSeconds()
                           << "s client received response " << packet->GetSize()
                           << " bytes from "
    ...
    if (payload[0] == uint8_t(MessageType::dataResponse)) {
      ...
      if (isReplication) {
        ...
        if (it != m_pending_reallocations.end()) {
          ...
          if (!m_realloc_ontime_CB.IsNull())
            m_realloc_ontime_CB(dataID, GetNode()->GetId(), diff);
          // log successful request
        } else {
          // log successful request, already gotten or late
          if (!m_realloc_late_CB.IsNull())
            m_realloc_late_CB(dataID, GetNode()->GetId(), diff);
        }
      } else {
        it = m_pending_lookups.find(origID);
        if (it != m_pending_lookups.end()) {
          m_pending_lookups.erase(it);
          if (!m_lookup_ontime_CB.IsNull())
            m_lookup_ontime_CB(dataID, GetNode()->GetId(), diff);
          // log successful request
        } else {
          // log successful request, already gotten or late
          if (!m_lookup_late_CB.IsNull())
            m_lookup_late_CB(dataID, GetNode()->GetId(), diff);
        }
      }

      NS_LOG_LOGIC("TODO: Mark cache miss, mark lookup success, remove from "
...
void SafApplication::LookupData(uint16_t dataID) {
  ...
  if (item.GetStatus() == DataStatus::stored) {
    if (!m_cache_hit_CB.IsNull())
      m_cache_hit_CB(dataID, GetNode()->GetId());
  } else {
...
void SafApplication::SaveDataItem(Data data) {
  ...
  for (std::vector<Data>::iterator it = m_replica_data_items.begin();
       it != m_replica_data_items.end(); ++it) {
    if (!found && m_access_frequencies[it - m_replica_data_items.begin()][0] ==
                      data.GetDataID()) {
      found = true;
    }
...
void SafApplication::AskPeers(uint16_t dataID, bool isReplication) {
  ...
  m_txTraceWithAddresses(
      p, localAddress, InetSocketAddress(Ipv4Address::GetBroadcast(), m_port));
  ...
  if (isReplication) {
    ...
    if (!m_realloc_sent_CB.IsNull())
      m_realloc_sent_CB(dataID, GetNode()->GetId());

    if (Simulator::Now() + m_request_timeout < m_stopTime) {
      Simulator::Schedule(m_request_timeout,
                          &SafApplication::ReallocationTimeout, this,
                          m.getRequestID());
    }
  } else {
    m_pending_lookups.insert(m.getRequestID()); // add to pending list
    // stats for 'normal lookup'
    if (!m_lookup_sent_CB.IsNull())
      m_lookup_sent_CB(dataID, GetNode()->GetId());

    if (Simulator::Now() + m_request_timeout < m_stopTime) {
      Simulator::Schedule(m_request_timeout, &SafApplication::LookupTimeout,
                          this, m.getRequestID());
    }
  }
...
void SafApplication::LookupTimeout(uint32_t requestID) {
  ...
  if (item != m_pending_lookups.end()) {
    if (!m_lookup_timeout_CB.IsNull())
      m_lookup_timeout_CB(requestID, GetNode()->GetId());
...
void SafApplication::ReallocationTimeout(uint32_t requestID) {
  ...
  if (item != m_pending_reallocations.end()) {
    if (!m_realloc_timeout_CB.IsNull())
      m_realloc_timeout_CB(requestID, GetNode()->GetId());
...
void SafApplication::RunReplication() {
  ...
  for (uint16_t i = 0; i < m_replica_space; i++) {
    ...
    if (!found)
      AskPeers(m_access_frequencies[i][0], true);
  }
